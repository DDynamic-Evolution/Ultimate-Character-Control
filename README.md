...
**Default-on items:** Append `_d` to an item folder name to mark it as the default for that category. It will auto-attach whenever that avatar is loaded — no manual selection needed.
...
| Notifications   | Toggle owner notification when someone touches your UCC          | Default: on    |
...
- A **Main** button is available on every page of multi-page menus (Outfits, Avatars, Addons, AvAddons). You do not need to scroll to the last page to return to the main menu.
...
* New Access menu buttons: AddBlacklist, AddWhitelist, RemBlacklist, RemWhitelist (UUID input via TextBox)
* Old blocked/permitted keys are automatically migrated to the new shared keys on first run
* .conf loading merges accesswhitelist/accessblacklist into the shared lists instead of overwriting
* .conf reading is skipped after a settings restore to prevent overwrite
* Full internationalization: all new Access menu strings translated in en, de, es, fr, pt
...
Version 2.05
* Added Notifications option — toggles the owner alert when someone touches your UCC
* Fixed: Notifications button in Options was incorrectly toggling PGSafe
* AvAddon default-on flag: append _d to an item folder name to auto-attach it when the avatar loads
* Fixed: AvAddon detach on avatar switch was not firing (typo in internal handler)
* Language file consistency fixes: all 7 language packs now include SET_MASTER, REMOVE_MASTER, OPT_STATUS_NOTIFICATIONS
